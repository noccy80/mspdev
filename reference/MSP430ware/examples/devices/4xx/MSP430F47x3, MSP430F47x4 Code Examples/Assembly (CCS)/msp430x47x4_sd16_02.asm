;******************************************************************************
;   MSP430x47xx Demo - SD16, Single Conversion on a Group of 4 Channels
;
;   Description: This program uses the SD16 module to perform a single
;   conversion on a group of channels (0, 1, 2 and 3). A SD16 interrupt occurs
;   when the conversions have completed. Test by applying voltages to the
;   4 input channels and setting a breakpoint at the line indicated
...
;   are stored in register R5, CH1 results are stored in register R6,
;   CH2 results are stored in register R7 and CH2 results are stored in
;   register R8.
...
SetupSD16   mov.w   #SD16REFON+SD16SSEL0,&SD16CTL ; 1.2V ref, SMCLK
            ...
            bis.w   #SD16SNGL+SD16IE,&SD16CCTL3   ; Single conv, enable int.
...
SD_CH3      mov.w   &SD16MEM3,R8            ; Save CH2 conversion result
