//******************************************************************************
//   MSP430F21x4 Demo - Timer_A, Ultra-Low Pwr Pulse Accumulator
//
...
void main(void)
{
  ...
  TA1CTL = TASSEL1 + TASSEL0 + TACLR + TAIE; // Ext. INCLK, interrupt
  TA1R = 0xFFFF - 100;                       // Offset until TAR overflow
  TA1CTL |= MC1;                             // Start Timer_A continuous mode
...
__interrupt void TA1_ISR(void)
{
  switch (__even_in_range(TAIV, 10))        // Efficient switch-implementation
  {
    case 2:
      ...
      break;
    case 10:
      TA1CTL = TASSEL1 + TACLR;              // SMCLK, clear TAR
      TA1CCTL1 = CCIE;                       // TACCR1 interrupt enabled
      TA1CCR1 = 50000;
      TA1CTL |= MC1;                         // Start Timer_A in continuous
