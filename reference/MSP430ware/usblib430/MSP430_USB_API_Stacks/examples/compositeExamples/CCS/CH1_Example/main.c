...
|                              Texas Instruments                              |
|                                                                             |
|                MSP430 USB-Composite-Example (CDC/HID Driver)                |
|                                                                             |
+-----------------------------------------------------------------------------*/
...
-------------------------------------------------------------------------------+
 ...
 and a HID application (MSP430 HID USB Application).
 The demo application demonstrates the interation between two USB devices 
 (CDC and HID) through a simple chat program between the two
 applications. Characters that are entered into one application are transmitted
 to the other application. Make sure both applications are running and 
 initialized before attempting to send data.  
...
volatile BYTE bHID_DataReceived_event = FALSE; // Flags set by event handler to indicate data has been received into USB buffer
           ...
           case ST_ENUM_ACTIVE:
                ...
                if(bHID_DataReceived_event) //Message is received from HID application
                {
                    ...
                    strncat(wholeString,(char*)dataBuffer,count);
                  	strncat(wholeString," \r\n ",4);
                	if(cdcSendDataInBackground((BYTE*)wholeString,strlen(wholeString),0,1)) // Send message to other CDC App
                	{
                		SendError = 0x01;
                		break;
                	}
                ...
                if(bCDC_DataReceived_event)  //Message is received from CDC application 
                {
                  ...
                  cdcReceiveDataInBuffer((BYTE*)wholeString,MAX_STR_LENGTH,0);
                  ASCII(wholeString);
                  if(hidSendDataInBackground((BYTE*)wholeString,strlen(wholeString),1,1))  // Send message to HID App
                  {
