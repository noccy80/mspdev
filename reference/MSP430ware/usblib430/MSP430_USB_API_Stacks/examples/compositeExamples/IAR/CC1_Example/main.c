           ...
           case ST_ENUM_ACTIVE:
                ...
                if(bDataReceived_event0) //CDC0
                {
                	...
                	memset(pieceOfString,0,MAX_STR_LENGTH);			// Clear pieceOfString
                  	cdcReceiveDataInBuffer((BYTE*)pieceOfString,MAX_STR_LENGTH,0);
                  	...
                  	if(retInString(wholeString))                     //Wait for enter key to be pressed
                  	{
                  		...
                  		if(cdcSendDataInBackground((BYTE*)wholeString,strlen(wholeString),1,1))   // Send message to other App
                  		{
			...
			memset(pieceOfString,0,MAX_STR_LENGTH);			// Clear pieceOfString
                  	cdcReceiveDataInBuffer((BYTE*)pieceOfString,MAX_STR_LENGTH,1);
                  	strncat(wholeString,pieceOfString,strlen(pieceOfString));   
                  	if(retInString(wholeString))                     //Wait for enter key to be pressed
                  	{
                  		strcat(wholeString,"\r\r\n");
                  		if(cdcSendDataInBackground((BYTE*)wholeString,strlen(wholeString),0,1))   // Send message to other App
                  		{
                    		  SendError = 0x01;                          // Something went wrong -- exit
                    		  break;
                  		}
