...
-------------------------------------------------------------------------------+
 ...
 For the demonstartion of this example use the UsbApp.exe PC application.

 Device sends the string "Press any key" continuously and this is displayed in 
...
VOID main(VOID)
{
    ...
    Init_StartUp();
    USB_init();
    USB_setEnabledEvents(kUSB_VbusOnEvent + kUSB_VbusOffEvent + kUSB_UsbSuspendEvent + kUSB_UsbResumeEvent + kUSB_dataReceivedEvent + kUSB_UsbResetEvent);
        ...
        switch(USB_connectionState())
        {
           ...
           case ST_ENUM_ACTIVE:
                if(!bHIDDataReceived_event)                         // Do this until a key is pressed
                {
                  ...
                  if(hidSendDataInBackground((BYTE*)pakOutString,strlen(pakOutString),0,0))   // Send it
                  {
                    USBHID_abortSend(&w,0);              // Operation probably still open; cancel it
                    break;                               // The break isn't really necessary since it'll go to the end of the loop anyway, but it's shown for completeness
                  }
                }
                else
                {
                  if(hidSendDataInBackground((BYTE*)bufferX,MEGA_DATA_LENGTH,0,0))
                  {
                  ...
                  if(hidSendDataInBackground((BYTE*)bufferY,MEGA_DATA_LENGTH,0,0))
                  {
                   	bHIDDataReceived_event = FALSE;
                    USBHID_abortSend(&w,0);              // Operation probably still open; cancel it
                    break;
                  ...
                  if(rounds++>=500)
                  {
                    ...
                    if(hidSendDataInBackground((BYTE*)outString,strlen(outString),0,0))  // Send it
                    {
                      USBHID_abortSend(&w,0);            // Operation may still be open; cancel it
                      break;
                    }
                    bHIDDataReceived_event = FALSE;
                    rounds = 0;
                    USBHID_rejectData(0);                // Reject data from previous keypress, in preparation for another one
