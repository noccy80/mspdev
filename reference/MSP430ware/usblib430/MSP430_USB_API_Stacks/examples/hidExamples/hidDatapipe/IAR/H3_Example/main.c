...
BYTE dataBuffer[BUFFER_SIZE];                       // User buffer to hold the data
    ...
    while(1)
    {
        ...
        switch(USB_connectionState())
        {
           ...
           case ST_ENUM_ACTIVE:
                ...
                {
                  bHIDDataReceived_event = FALSE;                                // Clear flag early -- just in case execution breaks below because of an error
                  count = hidReceiveDataInBuffer(dataBuffer,BUFFER_SIZE,0); // Count has the number of bytes received into dataBuffer
                  if(hidSendDataInBackground(dataBuffer,count,0,1))             // Echo is back to the host
                  {
