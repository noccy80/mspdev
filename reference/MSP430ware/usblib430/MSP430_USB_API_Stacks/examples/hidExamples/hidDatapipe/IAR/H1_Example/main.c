        ...
        switch(USB_connectionState())
        {
           ...
           case ST_ENUM_ACTIVE:
                ...
                if(bHIDDataReceived_event)                              // Some data is in the buffer; begin receiving a command              
                {
                  ...
                  hidReceiveDataInBuffer((BYTE*)pieceOfString,MAX_STR_LENGTH,0);                 // Get the next piece of the string
                  strcat(wholeString,pieceOfString);     
                  // Add it to the whole
                  hidSendDataInBackground((BYTE*)pieceOfString,strlen(pieceOfString),0,0);      // Echoes back the characters received (needed for Hyperterm)
